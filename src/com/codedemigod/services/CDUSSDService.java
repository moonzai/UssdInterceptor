...
public class CDUSSDService extends Service {
	...
	public static final String INTENT_USSD = "com.codedemigod.services.INTENT_USSD";
	public static final String INTENT_ACTIVATE = "com.codedemigod.services.INTENT_ACTIVATE";
	public static final String INTENT_DEACTIVATE = "com.codedemigod.services.INTENT_DEACTIVATE";

	BroadcastReceiver receiver = new BroadcastReceiver() {
		@Override
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(INTENT_ACTIVATE)) {
				// activity wishes to listen to USSD returns, so activate this
				mActive = true;
				Log.d(TAG, "activate ussd listener");
			} else if (intent.getAction().equals(INTENT_DEACTIVATE)) {
				mActive = false;
	...
	private final IExtendedNetworkService.Stub mBinder = new IExtendedNetworkService.Stub() {
		...
		public CharSequence getUserMessage(CharSequence text)
			...
			// text.toString()).build();
			// sendBroadcast(new Intent(Intent.ACTION_GET_CONTENT,
			// ussdDataUri));

			Intent ussd = new Intent(INTENT_USSD);
			ussd.putExtra("extra", text);
			sendBroadcast(ussd);
	...
	public IBinder onBind(Intent intent) {
		...
		registerReceiver(receiver, new IntentFilter(INTENT_ACTIVATE));
		registerReceiver(receiver, new IntentFilter(INTENT_DEACTIVATE));
